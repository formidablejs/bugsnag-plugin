...
class BugsnagServiceResolver extends _$frameworkφ4.ServiceResolver {
	...
	boot(){
		...
		if (!((_$frameworkφ3.helpers.isEmpty(this.app.config.get('bugsnag.api_key'))))) {
		...
		_$jsφ.default.registerCallback = function(handler) {
			...
			if (_$frameworkφ3.helpers.isEmpty(_$jsφ.default._registered_callbacks)) {
	...
	catchQueries(){
		...
		return self.app.addHook('onRequest',function(request,reply,done) {
			...
			_$frameworkφ.Database.on('query',function(data) {
				...
				if (_$frameworkφ3.helpers.isEmpty(request._knex_data)) {
	...
	*/
	setUser(request){
		...
		return this.event.addMetadata('user',{
			..._$frameworkφ3.helpers.without(user,this.app.config.get('auth.providers.jwt.hidden'))
		});
	}
	...
	catchErrors(){
		...
		return self.app.onResponse(function(error,/**@type {FormRequest}*/request) {
			...
			if (error instanceof Error) {
				...
				if (!(_$frameworkφ3.helpers.isEmpty(self.app.handler)) && self.app.handler.shouldReport(error)) {
